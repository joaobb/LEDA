...
public class RepositorioProdutoNaoPerecivelArray {
    ...
    public void atualizar(ProdutoNaoPerecivel produto) {
        ...
        if (codProd != -1) {
            this.produtos[this.procurarIndice(codProd)] = produto;
        } else {
    ...
    public void remover(int codigo) {
        int indiceProd = this.procurarIndice(codigo);

        if (indiceProd != -1) {
            this.produtos[indiceProd] = this.produtos[this.index];
    ...
    public ProdutoNaoPerecivel procurar(int codigo) {
        ProdutoNaoPerecivel produtoRetornado;
        int indiceProd = this.procurarIndice(codigo);

        if (indiceProd != -1) {
            produtoRetornado = this.produtos[indiceProd];
        } else {
            throw new RuntimeException("Produto nao encontrado!");
        }
